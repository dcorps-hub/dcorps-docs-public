# dCorps Hub Master Reference (Internal)

#### Web3 native base layer for decentralized corporations and nonprofits

**Document type**: Master Reference (source of truth for design, specs, and build)  
**Doc ID**: DCHUB-MASTER-2025-12-02  
**Version**: v0.1  
**Status**: internal working draft  
**Confidentiality**: internal only (do not distribute externally)  
**Owner**: Nicolas Turcotte (Founding Architect)  
**Source repo**: dcorps-docs  
**Repo path**: /docs/master/DCHUB_MASTER.md  
**Build tag**: DCHUB-MASTER-2025-12-02-v0.1  
**Derived artifacts**: specs, policies, token and security docs, plus generated schema exports (see section 18.4)  
**Changelog**: maintained starting v0.1 in `/docs/master/CHANGELOG.md`

Contact: dev@dcorps.com

---

## Internal use notice and disclaimer

### Internal use notice

This document is an internal Master Reference used to design and implement the dCorps protocol and its related specifications, policies, and reference tooling.

- Not for public distribution, marketing, fundraising, or reliance by third parties.
- May contain incomplete sections, placeholders, and open questions.
- Where this document conflicts with a versioned normative spec (Protocol Specification, Module Protocol Standard, etc.), the normative spec controls.

### External style disclaimer (kept for safe wording)
...
This document describes a technical and economic design for the dCorps protocol as currently envisioned. Many details will change with engineering work, legal advice, market conditions, and community input. Any token sale, equity financing, or legal agreement will be governed by its own dedicated documentation and terms, not by this document.
...
## Document control (how to use this master)

### Authority and intent

This Master Reference:

- defines the intended system behavior, terminology, and boundaries,
- is the internal source of truth for design discussion, implementation planning, and reference tooling behavior,
- is the source for generating and maintaining v0.x normative specs and schemas.

### Claim labels (required)

Every major claim or requirement should be prefixed with one of:

- [FACT-IMPLEMENTED]
- [SPEC-NORMATIVE]
- [INTENT-PLANNED]
- [OPEN-QUESTION]
- [RISK-NOTE] (internal only)

### Conformance language

Words like “must” and “required” are used only for compatibility requirements and only when the requirement is also captured in a normative spec file under `/docs/spec/`.
...
- **Regulators, policymakers, and legal professionals**
   Read section 0.3B, section 4.4A, section 4.6A, section 14.3, and section 17.

#### Architecture at a glance (conceptual)

```text
External applications (UI, markets, payroll, donation portals, dashboards)
        |
        v
Protocol modules (jurisdiction, sector frameworks, attestations, reputation)
        |
        v
dCorps Hub (registry, entity models, roles, governance, wallet primitives, anchoring)
        |
        v
DCHUB staking and consensus, security root for Hub aligned sub chains
        |
        v
Recognized sub chains (public/private) that anchor summaries back to the Hub
```
...
**3. Attach modules or promote to a sub chain**

1. Attach a protocol module (jurisdiction module, sector framework, attestation module) via governance.
2. If needed, register a sub chain and start anchoring cap table and reporting summaries to the Hub.
3. If a recognized sub chain fails standards, its recognition label changes and downstream tooling responds deterministically.
...
dCorps is a Web3 native base layer for **corporations** and **nonprofit organizations**.

It provides:

- A **Cosmos based Hub chain** that acts as a global registry and coordination layer for entities.
- Standardized **on chain models** for:
  - Private corporations (Hub corporations with internal units).
  - Public corporations (sub chain corporations with dShares).
  - nonprofits and NGOs (board governed, with transparent donation and program flows).
- A common **wallet and accounting structure** that makes entity level income, spending, governance, and ownership verifiable over time.
- Open **interfaces and data standards** that protocol modules, jurisdictions, DeFi protocols, and applications can build on.

In the first years, dCorps is primarily a base registry, metadata layer, and Explorer for entities. The focus is to standardize how entities are registered, how their wallets and flows are modeled, and how their data appears in the Explorer. Applications and services then build on this shared base.

The native dCorps Hub token **DCHUB** is used for:

- Gas on the Hub.
- Staking and proof of stake security on the Hub, and as the security root for Hub aligned sub chains.
- Protocol level governance.
- Sub chain recognition economics where applicable (for example recognition deposits, bonds, and security fee payments defined by protocol standards).

Recognized sub chains may accept transaction fees in DCHUB or in other denominations depending on their explicit recognition tier. Regardless of fee UX, Hub aligned sub chains preserve DCHUB as the economic security root.

DCHUB is not equity in any corporation or nonprofit and does not represent a right to business profits or donations.

dCorps is **infrastructure**, not a bank, broker, exchange, or fund (see section 4.6). It does not issue or manage corporation equity instruments such as dShares on behalf of entities, and it does not provide legal, tax, or regulatory guarantees.

It defines a common programmable substrate where serious entities can be created, operated, and evaluated in a consistent and verifiable way.

A nonprofit **dCorps foundation** is intended to steward the protocol over time. Its mission focuses on:

- Keeping the Layer 1 Hub minimal, neutral, and purely Web3.
- Researching, developing, and proposing **protocol modules** that connect on chain entities to the real world, such as smart jurisdiction modules and sector frameworks.
- Supporting an open ecosystem of applications and tools, including an app and module registry that can function as a dCorps App Store.

Applications and protocol modules are developed by many parties. The foundation coordinates and supports modules that connect to law and society, while the core Hub remains stable and focused on entity logic.

---

### 0.1.1 In practice, what dCorps gives entities

In practice, dCorps is an **entity ledger for stablecoin native startups and NGOs**. In v1, the Hub itself stays focused on registry plus standardized wallet and tagging primitives; the “operating stack” emerges through applications and modules built on these primitives. Early versions function as a **stablecoin first operating stack on Cosmos**, with standardized entity wallets, a minimal standard chart of accounts, and tagged flows that can be summarized into simple **cash based operating reporting**. Smart jurisdiction modules, public sub chains, and more complex products arrive later on top of this base.
...
dCorps is explicitly optimized for entities that can route the large majority of their material activity through on chain wallets in stablecoins. Entities that require heavy dependence on bank rails can still use the registry and governance features, but dCorps does not attempt to be a bank integration layer in v1.

Concretely, dCorps gives entities:

- A **standard entity operating model** for corporations and NGOs, including a consistent wallet structure and machine readable flows.
- A way to express **governance, cap tables, and program allocation rules as verifiable state**, rather than only narrative policies, PDFs, or private spreadsheets.
- A **Web3 native operating posture** where counterparties, platforms, and protocols can treat the entity’s on chain state as the canonical record for authority, ownership, and approvals, even before any jurisdiction module is attached.
- A **gradual path to more formality**: start as a Hub corporation or Hub nonprofit on the shared Hub, then attach smart jurisdiction modules when needed, and if needed promote to a sub chain without rebuilding identity, history, and reporting from scratch.
- A **jurisdiction-portable core record**: because the primary entity, role, and ledger state live on the Hub, an organization can change which smart jurisdiction module it attaches to over its lifetime—such as starting with a US-oriented module, later withdrawing that attachment, and subsequently attaching to a different jurisdiction’s module—without recreating its internal structure, cap table, or operating history on chain. Off chain legal work and filings remain required in every jurisdiction; the Hub record provides continuity rather than replacing local law.
- For NGOs and foundations, **donation and allocation transparency** that donors, auditors, and regulators can verify directly from the ledger at a category level.
- For jurisdictions and service providers, a **neutral registry and accounting substrate** they can plug into instead of operating their own bespoke chain or closed database.
- A practical usability path where entities can operate without directly managing DCHUB day to day, using fee grants, sponsored transactions, or relayers (where supported by protocol standards and applications), while DCHUB remains the underlying staking and gas asset for the network.

The protocol’s on chain behavior and data models are designed to be consistent globally. Legal recognition, filing obligations, reporting formats, and enforcement remain jurisdiction dependent and are handled through optional smart jurisdiction modules plus matching off chain legal processes and documentation.
...
- Donors, counterparties, and regulators increasingly demand machine readable evidence, not just PDFs, for governance decisions and financial allocation claims.
...
This wedge is intentionally narrow. It proves that the Hub can be a neutral entity registry plus a standardized operating ledger for stablecoin native organizations. Jurisdiction modules, sub chains, and advanced privacy are later layers built on this foundation, not prerequisites for v1 usefulness.

### 0.1.2 v1 scope box (ships on mainnet, explicitly out of scope)

dCorps is a long term multi layer vision. Mainnet v1 is intentionally narrow and conservative so the Hub can stabilize as a neutral registry and entity operating substrate.

**In scope for v1 mainnet**

- Entity registry, entity IDs, entity metadata, and lifecycle status.
- Hub corporation module:
  - Ten thousand unit cap tables
...
- Standard wallet types and accounting primitives:
  - Canonical wallet type mapping per entity
  - Tagged accounting events and minimal default chart of accounts
  - Cash based operating reporting derived from tagged flows (see section 9.5B and section 9.8)
- Fee model and usability primitives:
  - DCHUB gas, staking, and protocol governance
  - Fee grants, sponsored transactions, and relayer compatible patterns
  - USDC based protocol service fees for registration and renewal
- Sub chain registry and anchoring interfaces (foundation for later growth):
  - Sub chain registration, recognition labels, and anchor submission events
  - Anchor schema versioning and failure handling standards
- Reference infrastructure and interoperability deliverables:
  - Reference Explorer and reference indexer behavior
  - Stable event streams, canonical queries, and export formats for third party tooling
  - Compatibility test suite for schema and module conformance

**Explicitly out of scope for v1 mainnet**
...
- Mandatory protocol level KYC, KYB, AML, sanctions screening, or travel rule enforcement (these live in applications, service providers, and optional modules).
- Full privacy execution as a default, including private zones and zero knowledge based reporting as a required baseline (supported as optional evolutions).

Everything out of scope for v1 is expected to arrive later through protocol modules, sub chains, and applications once the Hub core is mature and operationally proven.
...
> Give anyone on earth the ability to create and run a serious, transparent Web3 native corporation or nonprofit in a recognized digital ecosystem, whether or not it is attached to a jurisdiction, and to optionally attach legal recognition where and when it is needed.
...
- **Gradual path to formality**
  Entities should be able to start simple and experimental, then attach to legal regimes and public markets as they grow, without rebuilding their infrastructure each time.

---

### 0.3 Web3 first, protocol modules for jurisdictions

dCorps is a **Web3 first entity base layer**. That means:

* The **Layer 1 Hub** focuses on:

  * Entity registration and identity.
  * Cap tables and board structures.
  * Wallet structure and accounting events.
  * Governance actions and document anchors.
  * DCHUB staking and protocol governance.
* **Jurisdictions, regulation, and sector rules** are handled by **protocol modules on top of the core**, not hard coded into the base protocol:

  * Smart jurisdiction modules encode how specific countries or regions treat certain entity types. These modules run on the Hub, or on tightly coupled chains that share security and data with the Hub.
  * Sector and impact frameworks encode domain specific standards, for example climate or education metrics, again as protocol modules that read and interpret Hub state.
  * Other protocol modules can implement allocation rules, eligibility criteria, or domain specific logic without changing the Hub itself.
* **Applications** are completely separate from the Hub:

  * Fundraising platforms, donation portals, DeFi protocols, accounting tools, and dashboards operate as independent applications.
  * They talk to the Hub and sub chains through wallets, APIs, and SDKs.
  * They can be built by any developer and surfaced in an open **dCorps App Store**, where DCHUB holders can signal trust and quality through governance votes.

The **dCorps foundation** is expected to work with jurisdictions, sector experts, and builders to research, design, and propose official protocol modules, especially smart jurisdiction modules and sector frameworks. These modules reflect law, regulation, and social needs that change over time. They must evolve frequently and be updated or replaced as society and law change. The foundation stewards this adaptive layer.

The base layer has one job: **be a neutral, robust, auditable home for entities and their core operations**. The core Hub logic is intended to remain conceptually stable over time. Everything that links on chain entities to the off chain world, including legal regimes and sector standards, is packaged as protocol modules that can be added, upgraded, or retired without rewriting the Hub.

Applications provide user experience on top of both the Hub and its modules. They can be built and operated by anyone, and they do not become part of consensus.

------

### 0.3A Web3 native entities and scope

dCorps is a Web3 native entity base layer, meaning entities are represented as persistent on chain objects with explicit roles, wallets, governance, and accounting primitives.
...
- Anchors for off chain documents and sub chain summaries

dCorps is not a legal wrapper service and does not replace legal responsibility. Legal status, filings, and enforcement remain off chain and are achieved through optional integrations such as smart jurisdiction modules plus matching legal processes and documents. The protocol can help express and automate reporting logic and fee obligations, but it does not guarantee compliance, and it does not replace legal, tax, or accounting advice.

Most serious entities will operate in a mixed environment for a long time. dCorps is designed so that on chain flows can be fully transparent and verifiable, and so that off chain records (bank statements, statutory books, audits) can be reconciled and anchored to the Hub where needed, without putting sensitive data or personal information on chain.

------

### 0.3B What a Hub entity is before jurisdiction recognition

A dCorps Hub entity is a Web3 native organizational object with persistent identity, explicit roles, governance, and standardized account structures recorded on chain.

Before any smart jurisdiction module is attached, a Hub entity is not automatically granted legal personhood or legally recognized corporate or charity status in any country. This does not make it unserious. In Web3, many high value organizations, including protocol teams, DEX markets, and stablecoin native service businesses, operate credibly based on code enforced rules, transparent records, and counterparty expectations, without being anchored to a single jurisdiction registry entry.

In this pre recognition state, a Hub entity is best understood as:

- An on chain registry entry plus on chain state machines (cap table or board, wallet structure, governance events, accounting primitives), and
- A set of off chain agreements and practices that participants may choose to adopt, reference, and enforce through traditional legal systems where applicable.

dCorps does not grant legal personhood by itself. Legal personhood and enforceability come from off chain law, contracts, and institutions that explicitly reference the entity’s on chain identifiers, governance records, and ownership or board state.

Without legal recognition or an off chain legal wrapper, participants should not assume limited liability exists. Individuals who sign contracts, control wallets, or act as representatives of a jurisdictionless Hub entity may be personally liable for obligations, losses, taxes, employment claims, or other liabilities, depending on the facts and the applicable law. Entities and participants should obtain legal advice before relying on jurisdictionless operation for significant activity.

At the same time, a Hub entity can still be highly meaningful inside Web3. Counterparties and platforms can choose to treat the Hub entity state as the source of truth for who can bind the entity, how decisions are approved, and how treasury policy is enforced. In practice, this can be sufficient for many purely digital ventures, protocols, and global teams whose primary relationships are with Web3 native customers, suppliers, contributors, and other on chain entities.

#### 0.3B.1 Default v1 operating pattern (no jurisdiction module)

In v1, the expected default pattern for serious teams is:

1. Register a Hub corporation or Hub nonprofit on chain.
2. Adopt governing documents off chain that reference:
   - The entity ID,
...
4. Use anchored documents, attestations, and reconciliation patterns only where needed for external counterparties, audits, or mixed rail operations.
...
- The role binding method used for authority (wallet based roles, DID based roles, or mixed), and how role reassignment and key rotation are recognized.
...
- The list of decision classes for which the Hub record is treated as canonical evidence of approval, including, at minimum, the following when used:
  ...
  - Sub chain recognition related actions where applicable.
...
- For high stakes off chain obligations, use an attached smart jurisdiction module or an explicit off chain legal wrapper that references the Hub record, so that signers are not relying on informal assumptions about liability.

dCorps can standardize evidence and authority records. It does not replace legal structure.

#### 0.3B.2 Transition to legal recognition

When a smart jurisdiction module exists and an entity chooses to attach it, the entity may become legally recognized under that module’s regime, subject to local law and any required off chain filings or signatures.

The module writes recognition status as on chain state and attestations; the legal effect comes from the jurisdiction’s law and contracts that bind recognition to those on chain facts.

#### 0.3B.3 Web3 native operation (jurisdictionless by default)

dCorps is designed so that entities can be legitimate and useful even without any jurisdiction module attached, as long as their relevant counterparties agree to rely on the Hub record.

Common patterns include:

- Web3 native service businesses and digital collectives that transact primarily in stablecoins and onboard customers globally.
...
This does not replace law. It is a practical Web3 native organizational posture that can be upgraded later into legal recognition through jurisdiction modules and matching off chain processes when and where that becomes desirable.

A jurisdictionless posture also has limits. Without legal recognition in a specific jurisdiction, an entity may have reduced ability to access traditional banking, sign certain off chain contracts, hire under local employment law frameworks, or hold regulated assets. dCorps is designed to support both modes: jurisdictionless operation where it is sufficient, and optional legal recognition where it is required.
...
No. The Hub is a registry and evidence layer for Web3 native entities. Legal personhood, limited liability, filings, and enforcement remain off chain and are achieved through optional smart jurisdiction modules plus matching legal documents and processes (see section 0.3B and section 14.3).
...
## 1. Purpose, scope, and Web3 philosophy

### 1.1 What this master document is

This Master Reference is:

- the internal source of truth for the dCorps protocol design,
- a consolidated map of architecture, protocol behavior, economics, governance, and boundaries,
- the upstream source from which normative specifications, schemas, and reference implementations are produced.

[SPEC-NORMATIVE] Any compatibility requirement stated in this Master Reference must be mirrored in a versioned normative document under `/docs/spec/`. If it is not mirrored, treat it as [INTENT-PLANNED] or [OPEN-QUESTION], not as a requirement.

This Master Reference is meant to be used together with:

- Protocol Specification (normative messages, state machines, event schemas)
- Protocol Parameters and Economics (bounded ranges, genesis defaults)
- Module Protocol Standard (attachment, versioning, required interfaces)
- Sub chain Anchoring Standard (commitments, cadence, invalid anchor rules)
- Reference Indexer Specification (canonical exports and endpoints)
- Governance and Treasury policy documents

---

### 1.2 What this Master Reference is not

This Master Reference is not:
...
The intended readers are internal contributors and approved partners working under controlled sharing:

- core protocol engineers and reviewers,
- module and application engineers building reference tooling and standards,
- security and audit partners (under NDA),
- product, operations, and validator enablement teams,
- internal legal, risk, and governance design contributors.

The language aims to be precise enough for implementation and audit, without assuming specialist background in cryptography or specific jurisdictions.

---

### 1.4 Web3 first philosophy

The design of dCorps follows a Web3 first philosophy:
...
The aim is to combine the programmability and auditability of Web3 with the durability and seriousness required by real corporations and nonprofits.
...
At the same time, legal status and legal effects are not universal. Legal recognition, filings, reporting obligations, and enforcement still depend on jurisdiction, contracts, and institutions. dCorps addresses this by keeping jurisdiction specific differences in **smart jurisdiction protocol modules** and matching off chain processes, rather than hard coding them into the core chain.

---

### 1.5 How to operate this document (internal workflow)

This document is intentionally comprehensive. Treat it as a binder.

Rules:

- Use stable section IDs and do not rename section anchors casually.
- When adding new protocol requirements, update the relevant `/docs/spec/` files in the same pull request.
- When a requirement is not yet ready for a spec, mark it [OPEN-QUESTION] and add it to the TBD register.

#### Excerpting and distribution controls

- This file is internal only and is not a shareable source outside dCorps controlled channels.
- Do not copy sections into decks, partner docs, or external systems without explicit approval from the document owner (or a delegated maintainer).
- If an excerpt is approved for sharing, scrub [RISK-NOTE] items and internal implementation details, and ensure any remaining claims are consistent with the current normative specs.

------

### 1.6 Terminology and conventions

This Master Reference uses the following conventions to keep wording and technical meaning consistent:
...
- **Protocol module**: On chain logic that attaches to, reads, and writes Hub entity state (for example smart jurisdiction modules and sector frameworks).
...
- “On chain” and “off chain” are written as two words.
...
- Entities that want strong transparency route the large majority of their material activity through on chain wallets; mixed rail entities use anchoring and reconciliation patterns to bridge gaps.
...
- Jurisdiction modules are optional and may be delayed, limited, or unavailable for long periods; legal recognition remains jurisdiction dependent and off chain (see section 0.3B and section 14.3).
...
There is a missing layer between traditional systems and raw Web3 DAOs.

---

### 2.3 Why Web3 is an appropriate base

Web3 infrastructure, used carefully, solves some core issues:
...
* Stable, neutral base layer governance suitable for regulators and institutions.

dCorps uses Web3 not as a speculation machine, but as a **programmable entity operating system**.

---

### 2.4 What a Web3 native entity stack looks like

A modern entity stack built on Web3 should be:
...
* Jurisdictions and regulators that want to work with transparent on chain entities instead of purely off chain structures.
...
- Register a **Hub corporation** and assign the ten thousand internal units between themselves and early contributors.
- Receive all income into a **merchant wallet** in USDC.
- Pay salaries and contractors, and cover infrastructure costs, from that wallet and a treasury wallet, tagging each payment with categories.
- At month end, generate a **cash based operating statement** directly from on chain flows, suitable for internal reporting and stakeholder transparency, and distinct from GAAP or IFRS reporting.
- When the business justifies it, attach a **smart jurisdiction module** that gives them legal status in a friendly country, with automatic fee collection.
- If they reach sufficient scale and legal clarity, they can launch a **public sub chain** and migrate to **dShares** as their equity and governance instrument.

The underlying history of ownership, operations, and governance stays continuous.

#### DAO maturing into a structured corporation

A protocol has run as a token voting DAO for two years, with a large community token and a multisig treasury.

On dCorps it:

- Registers a **Hub corporation** to formalize its core team and legal responsibilities.
- Maps its existing community agreements into a structured cap table with units and vesting.
- Uses dCorps modules to run payroll, grants, and vendor payments with clear tagging and approvals.
- Over time, migrates to a **public sub chain corporation** with dShares, while keeping the community token for narrow governance or incentive roles if desired.

This gives the project a path toward long term sustainability and regulatory clarity.

#### Radically transparent NGO
...
- Defines program wallets and **allocation rules as code**, for example minimum percentages for direct program spending versus overhead.
- Allows donors to see, in near real time, how funds move from donation to program costs, and how board decisions are taken.

This does not replace field level due diligence, but it makes financial transparency and governance visible in a way traditional tools do not.
...
- Register a **Hub corporation** or a **private sub chain corporation** as a joint venture or SPV, with its own units or dShares.
- Allocate units or dShares between the parent entities according to their agreement.
- Route project revenues and costs through dedicated wallets, tagged separately from each parent entity’s own operations.
- Use dCorps reporting to give both parents and external financiers a clear view of the JV’s performance.

The JV or SPV behaves like any other dCorps corporation from a protocol point of view, while remaining ring fenced from each parent’s main operations.

#### Digital friendly jurisdiction as a protocol module

An offshore jurisdiction wants to offer a digital corporation regime without building a chain from scratch.

With dCorps it:

- Publishes a **smart jurisdiction module** as a protocol module on the Hub that recognizes certain Hub entities as legal corporations or charities.
- Encodes registration and renewal fees in USDC, automatically collected from entities that opt in.
- Splits those fees between a jurisdiction wallet and the dCorps Protocol Treasury.
- Offers regulators dashboards based on entity state, without hosting their own ledger.

Entities choose this module when they want that legal status. The jurisdiction gains a modern product. dCorps remains a neutral base.

------

### 2.7 Concrete operating examples with numbers

The following examples are simplified cash flow and tagged ledger illustrations. They are not intended to represent GAAP, IFRS, or any local statutory reporting standard.

#### 2.7.1 Hub corporation, monthly operations example

A Hub corporation routes all material revenue and operating payouts through its dCorps wallets.

**Month inputs**

- Subscription revenue received to merchant wallet: 50,000 USDC

**Month outputs (merchant wallet)**

- Salaries (payroll batch): 30,000 USDC
- Contractors: 5,000 USDC
- Cloud and infrastructure: 4,000 USDC
- Other operating expenses: 2,000 USDC

**Jurisdiction fees (set aside in cash)**

- Move to a fee reserve wallet: 500 USDC

**Treasury allocation (internal transfer, not an expense)**

- Transfer to treasury wallet for reserves: 5,000 USDC

**Tagged P&L view for the month (illustrative)**

- Revenue: 50,000 USDC
- Operating expenses: 41,000 USDC
- Jurisdiction fees: 500 USDC
- Net operating result: 8,500 USDC

**End of month balances (illustrative)**

- Merchant wallet remaining: 3,500 USDC
- Treasury wallet: 5,000 USDC
- Fee reserve wallet: 500 USDC

All payments carry tags such as `category`, `vendor_type`, `program`, and references to anchored invoices or agreements. Dashboards can compute expense breakdowns, cash runway, and period summaries directly from the tagged flows.

#### 2.7.2 Hub nonprofit, monthly allocation example

A Hub nonprofit receives donations into its donation wallet and allocates funds to program and support categories with board visibility and enforcement.

**Month inputs**

- Donations received to donation wallet: 100,000 USDC

**Month allocations (donation wallet outflows)**

- Program A direct costs: 58,000 USDC
- Program B direct costs: 17,000 USDC
- General and administrative overhead: 15,000 USDC
- Fundraising costs: 5,000 USDC

**Treasury retention (end of month buffer)**

- Retained in treasury: 5,000 USDC
...
Allocation rules can enforce constraints such as minimum program spending ratio over a rolling window, maximum overhead ratio, or restrictions on designated funds. Donors can verify inflows and category level outflows directly from the ledger and associated governance events.
...
Most serious entities today live at layers 1 to 3. Web3 systems often operate separately, focusing on DeFi and speculative tokens.

dCorps sits between these worlds as a **Web3 layer for real entities**. It is not a replacement for law or for all software, it is a shared base that both traditional and Web3 systems can plug into.
...
### 3.3 Web3 competition

Web3 competition and adjacent projects include:
...
* Offer clear, entity centric data standards that regulators and accountants can read without learning a new DAO dialect.
...
* **Protocol module responsibilities**:
  ...
  * Smart jurisdiction modules that encode local law and recognition logic.
...
* Smart jurisdiction modules express rules about which types of entities are recognized, what fees apply, and what reporting is expected.
...
- **Who is responsible**
  - Entities, application operators, issuance platforms, custodians, exchanges, payment providers, and smart jurisdiction module operators are responsible for implementing and complying with any KYC, KYB, AML, sanctions, travel rule, reporting, licensing, and consumer protection obligations that apply to their activities and jurisdictions.
...
This is essential for regulators, entities, and serious users to trust that the base layer behaves predictably over time.
...
* **dCorps does**:
  ...
  * Expose state in a way that auditors, donors, investors, and regulators can analyze.
* **dCorps does not**:
...
The dCorps ecosystem has four main conceptual layers:

1. **dCorps Hub chain (Hub)**

   * A Cosmos based chain that:

     * Hosts the global entity registry.
     * Stores entity structural state and lifecycle events.
     * Anchors public sub chains.
     * Runs DCHUB as gas, staking, and governance token.
2. **Hub entities**

   * Corporations and nonprofits that live entirely on the Hub:

     * **Hub corporations** with ten thousand internal units as their cap table.
     * **Hub nonprofits** with board based governance and transparent financial flows.
3. **Public and private sub chains**

   * Cosmos based chains that serve individual entities or small groups:

     * Public sub chain corporations issue dShares as equity style tokens.
     * Private sub chain corporations use dedicated chains for performance or privacy but may not issue public equity.
4. **Protocol modules and external applications**

   * **Protocol modules on top of the Hub**:

     * Smart jurisdiction modules that express how specific jurisdictions treat entities.
     * Sector and impact frameworks that define domain specific standards and metrics.
     * Other on chain rule sets that read and write Hub entity state.
   * **External applications**:

     * Fundraising and donation platforms.
     * DeFi integrations and indices.
     * Explorers, dashboards, accounting tools, compliance tools, and other user experience layers that interact with wallets and APIs.

The Hub is intentionally conservative and narrow. Sub chains and protocol modules are where most specialization happens. External applications provide human facing experiences and connect dCorps to the wider world.
...
* Assigning entity IDs and maintaining an entity registry.
* Hosting Hub corporation and Hub nonprofit modules.
* Recording sub chain registrations and their anchoring status.
* Running DCHUB staking and protocol governance.
* Providing the basic accounting and wallet primitives that all entities share.

Sub chains are responsible for:

* Their own transaction throughput and custom logic.
* dShares issuance and sub chain level corporate actions for public sub chain corporations.
* Local fees and operations that do not need to touch the Hub directly.

Protocol modules on top of the Hub are responsible for:

* Expressing jurisdiction specific rules as smart jurisdiction modules.
* Defining sector metrics and eligibility criteria as sector frameworks.
* Monitoring and interpreting entity state according to their own rule sets, for example allocation rules or eligibility for certain programs.

External applications are responsible for:

* User interfaces and experience.
* Integration into existing tools and workflows such as accounting systems and HR systems.
* Optional custodial services subject to regulation.
* Running markets, matching engines, issuance flows, and donation portals where applicable.

This separation of responsibilities keeps the system understandable and composable. The Hub is the neutral core, protocol modules are structured on chain add ons, and applications are flexible off chain clients.

The dCorps foundation focuses its work on:

* Specifying and stewarding protocol modules that connect entities to real world law and sector standards.
* Enabling an open application ecosystem without centralizing control of the Hub.
...
A typical corporation lifecycle on dCorps:

1. **Formation**

   * Launches as a **Hub corporation** with ten thousand units.
   * Defines governance template and key roles.
2. **Early operation**

   * Uses Hub wallets and modules to handle income, expenses, payroll, and basic governance.
   * May integrate accounting and HR tools from the ecosystem.
3. **Jurisdiction attachment (optional)**

   * Attaches to a smart jurisdiction module to gain legal recognition in one or more places.
   * Fee collection and basic reporting are automated through the module.
4. **Promotion to public sub chain (optional)**

   * When business size, legal clarity, and community interest justify it, the corporation:

     * Registers a sub chain as a **public sub chain corporation**.
     * Maps Hub units to dShares.
     * Migrates part of its operations to the sub chain while continuing to anchor summaries to the Hub.
5. **Maturity and evolution**

   * Continues to use dCorps for governance, accounting, and anchoring.
   * May support or integrate with sector frameworks, DeFi protocols, and indices.

At no point is the corporation forced to move to a sub chain. Small and medium entities can remain Hub corporations indefinitely.

---

### 5.5 Example lifecycle: NGO and umbrella sponsorship

A typical NGO lifecycle might look like:

1. **Formation**

   * Registers as a **Hub nonprofit** with a board and basic governance rules.
2. **Early operation**

   * Receives donations into a donation wallet.
   * Sets up program wallets and allocation rules.
   * Uses dashboards to show donors how funds are allocated.
3. **Umbrella sponsorship**

   * For small organizations, an existing larger NGO or foundation can act as an **umbrella sponsor**:

     * The sponsor is itself a dCorps nonprofit.
     * It may host sub programs as dedicated program wallets and combined reports.
     * Smaller NGOs can benefit from its reputation and compliance setup.
4. **Jurisdiction attachment**

   * Where charity or foundation law modules exist, the NGO can attach to smart jurisdiction frameworks that recognize it as a local charity.
5. **Growth and specialization**

   * Larger NGOs may eventually operate their own sub chains for performance or privacy while still anchoring core data to the Hub.

This path supports a long tail of NGOs while giving donors and regulators a consistent, verifiable view of flows and governance.

------

### 5.6 Why a dedicated Hub chain and token, not only contracts

It is natural to ask why dCorps needs its own Hub chain and token instead of being a smart contract suite or rollup on an existing chain.

dCorps runs a dedicated Hub chain on Cosmos and a native token, DCHUB, for several reasons:

- **Entity model in core, not as an afterthought**

  The Hub treats entities as first class objects: corporations and NGOs, with registry entries, wallet structures, governance state, and accounting primitives at protocol level. This is more than a set of contracts. It is a chain whose core data structures and APIs are shaped around entity lifecycles, not generic accounts.

- **Shared security and coordination for sub chains**

  dCorps expects a landscape of public and private sub chains that serve specific corporations and NGOs. Recognized sub chains anchor their summaries back to the Hub and can align to Hub security under explicit recognition tiers. A dedicated Hub with DCHUB staking provides the common security root and coordination layer for Hub aligned sub chains, rather than a loose collection of unrelated contracts and chains.

- **Neutrality with respect to external host chain governance**

  A protocol that wants to support many jurisdictions, NGOs, and corporations over decades must not be at the mercy of another chain's governance, fee changes, or political shifts. Running a dedicated Hub keeps dCorps neutral. Other chains remain important partners, but they are not the home of the entity registry or the final record of entity state.

- **Consistent economic and security model**

  DCHUB is the gas, staking, and governance token for the Hub. For recognized sub chains, DCHUB remains the economic security root through shared security or other governance approved aligned security mechanisms, and through recognition economics such as DCHUB deposits, bonds, or security fee payments where defined by protocol standards. Sub chains may accept user transaction fees in DCHUB or in other denominations depending on the explicit recognition tier.

- **Interoperability as a feature, not a constraint**

  By running as a Cosmos Hub style chain, dCorps can connect through IBC to stablecoin issuers, DEX chains, privacy zones, and other app chains. The entity registry, NGO flows, and governance records live on a chain designed for that purpose, while bridges bring in liquidity and external applications. The protocol is free to integrate with multiple ecosystems rather than being tied to a single host.

- **Clear responsibility boundaries**

  A dedicated Hub with its own validator set, governance, and foundation makes it easier for regulators, NGOs, and serious entities to understand who is responsible for what. A pure contract suite on a shared host chain would blur those lines, since base chain governance could change conditions in ways that are hard to predict for entity level users.

For many users, the Hub will feel like a specialized rollup: a focused execution environment that still connects to the rest of Web3. The difference is that entity structures and NGO flows are native to the chain, and DCHUB staking and governance are aligned with that purpose rather than with a generic host.
...
The Hub is the coordination heart of the ecosystem:

* It is the **canonical record** of:

  * Which entities exist.
  * Their basic type and status.
  * Their attachments to protocol modules and sub chains.
* It provides a secure environment for:

  * Private Hub corporations and nonprofits.
  * Protocol level governance.
  * Anchoring of sub chains and documents.

The Hub is intentionally simpler and more conservative than the applications and modules built on top of it.
...
- Assigns each entity:
  ...
  - A type (Hub corporation, Hub nonprofit, public sub chain corporation, private sub chain corporation).
- Stores key metadata:
  - Sector tags.
  ...
  - Status (active, inactive, dissolved, retired).
- Emits events for:
...
- **dissolved**
  - The entity has explicitly ended operations through an on chain governance action, a jurisdiction module action, or another defined termination path.
...
- Inputs may include:
  ...
  - Outputs from smart jurisdiction modules (recognition active or withdrawn).
...
### 6.4 Sub chain anchoring

Sub chains recognized as part of the dCorps ecosystem:

- Periodically commit **Merkle roots** or similar commitments to the Hub.
- Include hashes that represent:
  - dShare total supply and cap table trees.
  - Aggregated revenue and expense summaries per period.
  - Governance checkpoints, such as full board elections or major resolutions.

Anchoring provides:

- A way for auditors, regulators, and DeFi protocols to verify that sub chains match their claims.
- A fallback for detecting forks or inconsistent behavior.

If a sub chain fails to anchor or violates standards, the Hub can mark it as **not recognized**, which signals caution to tools, protocol modules, and jurisdictions, without preventing the chain from operating independently.

The normative cadence, schemas, proofs, and failure handling rules for anchoring are defined in the **Sub chain Anchoring Standard** (see section 18.4).
...
#### 6.5A Sub chain recognition labels (registry standard)
...
#### 6.5B Sub chain aligned security (default shared security model)
...
#### 6.5B.1 Recognized, Hub aligned security (DCHUB gas)

A sub chain qualifies for “Recognized, Hub aligned security (DCHUB gas)” when all conditions below are met:

- **Shared validator security**
  The sub chain uses the Hub validator set through a shared security mechanism approved by governance. Validator security is rooted in DCHUB stake and the Hub’s slashing framework, with consumer chain evidence routed through the shared security protocol.
- **DCHUB gas**
  DCHUB is an accepted fee denomination at the base fee layer and is the primary default gas token for the sub chain.
- **Mandatory anchoring**
...
A sub chain qualifies for “Recognized, Hub aligned security (flexible gas)” when all conditions below are met:

- **Shared validator security**
  The sub chain uses the Hub validator set through a shared security mechanism approved by governance. Validator security is rooted in DCHUB stake and the Hub’s slashing framework, with consumer chain evidence routed through the shared security protocol.
- **Flexible fee denominations**
  The sub chain may accept transaction fees in one or more non DCHUB denominations at the application layer or fee market layer, with clear disclosure of supported fee denoms and conversion or sponsorship behavior where applicable.
- **DCHUB security rent (required)**
...
This approach balances the need for evolution with the reliability that entities, NGOs, and regulators require.
...
Each Hub corporation has exactly **10,000 internal units** that form its cap table.

Ten thousand is a deliberate design choice:

- It keeps the mental model simple. One unit is 0.01 percent of the corporation. A partner who owns 34.54 percent holds 3,454 units.
- It makes Hub corporations easy to compare and reason about, since every private corporation on the Hub shares the same total unit count.
- It discourages unit inflation games and awkward decimals, which are common problems with arbitrary token supplies.

These 10,000 units are the internal equivalent of ownership in a traditional LLC or Inc style structure:

- Units represent:
  - Economic rights such as entitlement to distributions and exit proceeds.
  - Voting power in shareholder level decisions.
  - Exposure to dilution when new units are issued.

Units are an internal ledger representation of rights defined by the corporation’s governing documents; their legal effect depends on off chain agreements and applicable law.

- By default, a simple **one unit, one vote** model applies for shareholder votes.
...
- External contracts and agreements can refer to unit holdings for economic calculations, but units do not circulate outside that entity context.

Units are a protocol cap table primitive. They are on chain state scoped to a single entity, used to represent ownership and voting relationships as defined by the entity’s governing documents and, where applicable, by attached jurisdiction modules and off chain legal processes. Whether a particular unit arrangement is treated as equity, a security, or another regulated instrument depends on facts and law outside the protocol. Transfer restrictions and approval workflows enforce on chain process and evidence trails; they do not, by themselves, guarantee legal compliance.
...
When structures become more complex, there are two main paths:

- Use standardized pools and claims on top of the 10,000 units for precision without changing the base model.
- Promote to a sub chain corporation when multiple share classes, public style trading, or complex waterfalls justify it.
...
Common corporate actions are modeled as first class operations:

- Splits and consolidations of units.
- Creation and adjustment of employee or contributor pools.
- Conversion of units in connection with financing, mergers, or restructurings.
...
This same mechanism supports joint ventures and SPVs. Two or more corporations can hold units or dShares in a dedicated entity that runs on the Hub or on a private sub chain. The JV or SPV uses the same corporate primitives as any other dCorps corporation, but its wallets, governance rules, and reporting are tailored to a specific project or asset and are clearly separated from each parent entity’s own operations.
...
### 7.2 Sub chain corporations and dShares
...
Smart jurisdiction modules can link these two worlds:
...
Even when a smart jurisdiction module exists, the **legal effect** comes from local law and contracts that refer to the module, not from the chain itself. The module is a technical expression of rules that the jurisdiction or its delegated providers choose to enforce.
...
Entities are free to launch independent Cosmos chains that use their own security assumptions and do not follow these standards. Such chains are not recognized as dCorps sub chains and do not benefit from the shared Explorer, protocol modules, and standardized tooling described in this Master Reference.

The Hub then:

- Marks the entity as a public sub chain corporation.
- Exposes its anchored data to explorers, modules, and tools.
- Allows smart jurisdiction and sector modules to reference it.
...
This gives donors, regulators, and auditors a clear picture of how decisions are made.
...
#### 7.3.6 Practical NGO operating pattern

In practice, many NGOs will operate in a mixed environment for some time, but dCorps is optimized for organizations that can run their primary flows on chain in stablecoins.

A practical operating pattern looks like:

1. The NGO receives donations and grants in USDC (originating from Noble and transferred to the Hub via IBC, interchain accounts, or relayers where supported) into its **donation wallet** on the Hub.
2. The NGO pays on chain expenses (staff, contractors, vendors, partner NGOs) directly from its canonical wallets wherever feasible, using standardized categories and governance controls.
3. If the NGO converts part of its holdings into local fiat through one or more regulated partners (exchanges, payment institutions, banks), those fiat payments occur off chain and are not part of Hub consensus.
4. Where required for audit, donor assurance, or internal controls, the NGO may anchor periodic standardized statements or third party attestations that reconcile specific off chain facts to the on chain reporting periods and commitments.

dCorps does not move fiat. External applications and service providers may offer optional bridges to local currencies, but the Hub remains the source of truth for digital balances and for the governance and reporting record that the NGO chooses to make public.
...
- **Simple private entity**
  *Deployment:* dCorps Hub (shared)
  *Complexity:* Low
  A straightforward private entity on the shared Hub with 10,000 internal units and USDC wallets for income and expenses. Built for remote first startups and small teams that want real governance, clear ownership, and transparent flows from day one, without running their own chain.
- **Simple nonprofit**
  *Deployment:* dCorps Hub (shared)
  *Complexity:* Low
  A Hub nonprofit with donation and program wallets plus clear allocation categories. Built for NGOs and small nonprofits that want transparent funding and spending, board based governance, and basic reporting, again without running their own infrastructure.
- **Private corporation**
  *Deployment:* Private sub chain
  *Complexity:* Medium
  A private corporation that needs a dedicated sub chain for custom share classes, richer controls, or specific performance and privacy requirements, while keeping critical state anchored to the Hub. This pattern fits entities that have outgrown the simple Hub template but do not yet need public trading.
- **Multi class private corporation**
  *Deployment:* Private sub chain
  *Complexity:* Medium to high
  A larger private corporation that needs multiple classes, complex rights, or bespoke governance logic. It operates on a dedicated sub chain with strong links back to the Hub for anchoring, discovery, and integration.
- **Public company**
  *Deployment:* Public sub chain
  *Complexity:* High
  A public company on a dedicated dCorps sub chain that issues dShares as equity style tokens. It operates at scale, supports liquid ownership, and keeps cap tables, governance checkpoints, and key financial summaries anchored to the Hub.
- **Sophisticated nonprofit**
  *Deployment:* Private sub chain
  *Complexity:* Medium
  A nonprofit that needs tailored program rules, regional or partner specific governance, and more complex multi wallet setups. It uses a private sub chain for flexibility and privacy while anchoring high level transparency, governance events, and allocation metrics to the Hub.

In the very first phase, dCorps focuses almost entirely on the two simplest Hub templates: the simple private entity and the simple nonprofit. Public companies and multi class structures arrive later as standardized sub chain templates.

These templates are not a fixed catalog. Governance can introduce new templates, refine existing ones, or deprecate patterns that do not match real world usage. The Hub and module architecture is designed so that new templates can be added as modules or standard entity types without rewriting core consensus logic.

------

### 7.5 Migration and promotion paths

dCorps is designed for gradual adoption. Entities can start by using the Hub as an operating layer, then attach legal regimes and specialized execution over time, without losing continuity of identity, governance history, or financial reporting.

#### 7.5.1 Using dCorps as an operating layer for an existing entity

An existing off chain corporation or nonprofit can adopt dCorps as a parallel operating ledger:

- Register a dCorps entity and bind roles (directors, officers, board seats) to DIDs and role wallets.
- Route selected revenue, donations, and payouts through the dCorps wallet structure.
- Anchor key off chain documents such as board minutes, contracts, audits, and ledger exports by hash.
- Reconcile off chain bank balances and statutory books to on chain balances using signed periodic reports and anchors.

This pattern improves transparency and process integrity without claiming that the on chain entity is automatically recognized as the legal entity in any jurisdiction.

#### 7.5.2 Attaching a smart jurisdiction module for recognition

When a suitable smart jurisdiction module exists, an entity can opt in:

- Attach the module on chain and accept its terms off chain where required.
- Provide required disclosures via anchored documents and on chain attestations.
- Pay jurisdiction fees in USDC according to the module.
- Receive a recognition status and compliance signals written by the module.

Legal effect comes from local law and contracts that reference the module, not from the chain alone.

#### 7.5.3 Promoting a Hub corporation to a public or private sub chain corporation

When needed for scale, privacy, or equity instruments, a Hub corporation can promote to a sub chain:

- Register the sub chain with the Hub and satisfy the technical recognition requirements.
- Produce and anchor a migration package that includes:
  - A snapshot of Hub unit balances and ownership proofs
  - A mapping plan from units to the initial dShare distribution (for public sub chains), or to the sub chain cap table model (for private sub chains)
  - Board and governance continuity references
- Activate the sub chain and begin periodic anchoring of cap table snapshots, governance checkpoints, and key financial aggregates back to the Hub.

The Hub entity ID and history remain the global reference, while execution and specialized logic move to the sub chain.

#### 7.5.4 Moving an NGO to a private sub chain

An NGO may migrate to a private sub chain to support:

- Higher transaction volume and program complexity
- Stronger privacy for beneficiary level detail
- Regional workflows and partner controlled execution

In this case, the NGO continues to anchor category level transparency, governance events, and reporting aggregates to the Hub so that donors, auditors, and regulators retain a consistent view.
...
- Protocol modules and applications can apply their own identity policies, for example requiring specific credentials for attaching to a smart jurisdiction module or for holding specific roles.
...
dCorps is transparent by design, but organizations have legitimate privacy needs. Privacy in a Web3 system has two distinct meanings:
...
To make choices explicit, each entity declares a disclosure mode at creation. The disclosure mode is public metadata and is used by explorers, registries, sector frameworks, and jurisdiction modules.
...
In the initial implementation, USDC on Noble is the reference stablecoin for reporting and protocol fees. In practice, entities hold and transact USDC on the Hub as an IBC transferred asset originating from Noble (and in some flows via interchain accounts or relayers where supported). Reports treat these balances as USDC denominated.
...
The protocol does not attempt to model fiat rails or bank payments as first class operations in v1. Entities that still touch fiat do so outside protocol consensus. Where an entity wants continuity of reporting across rails, it may optionally anchor standardized period statements or third party attestations that reference off chain records, without turning the Hub into a banking integration layer.
...
#### 9.5A.3 Reconciliation and completeness signals (optional)

dCorps does not require off chain reconciliation to function. However, entities that operate across multiple rails may voluntarily publish:

- Period commitments (hashes of standardized exports)
- Third party attestations that specific off chain facts reconcile to the on chain period record

These signals are optional and exist primarily to serve auditors, large donors, and regulated counterparties. Reference dashboards must clearly distinguish self reported views from views supported by third party attestations.
...
- Jurisdiction module outputs that required reports and fees were satisfied, or that recognition is active or withdrawn.
...
- Sector and jurisdiction modules to express additional rules without losing comparability.
...
Not all CBDCs will be compatible with open Web3 systems. Many may be permissioned, non transferable, or restricted in ways that prevent direct integration. dCorps supports CBDC style assets only where their technical rails and legal constraints allow safe, auditable use.
...
- Used for protocol level governance, within the scopes defined in this Master Reference and related governance charters.
...
- Used for many protocol level fees such as entity registration and renewal, premium names, and participation in smart jurisdiction modules.
...
- **Jurisdiction pilots and module development**: 6 percent (60,000,000)
  - Smart jurisdiction pilots, standards work, legal research support, jurisdiction module implementation grants.
...
In this Master Reference, the **Token Generation Event (TGE)** is the same event as **mainnet launch**, defined as the first production block of the dCorps Hub mainnet, when initial DCHUB allocations become live on chain and DCHUB becomes usable for gas and staking.

In this Master Reference, “emissions” refers to **tokens entering circulation** from predefined allocations (for example the staking and validator rewards pool), not the creation of supply beyond the fixed cap defined in section 10.3.
...
- **Staking and validator rewards (18 percent)**
  ...
  - Governance can adjust distribution parameters within predefined bounds (see section 10.6A), with stronger requirements for changes that materially increase near term emissions
...
Governance may adjust the schedule within predefined bounds (see section 10.6A). Changes that materially increase near term emissions beyond those bounds are designated as Protected Changes under the governance framework.
...
- Official smart jurisdiction module participation.
...
- A share to participating jurisdictions where smart jurisdiction modules are used.
...
- Deposits or bonds required for official smart jurisdiction modules and sector frameworks.
...
### 10.6A v1 protocol parameters snapshot (initial ranges)
...
**Sub chain recognition economics (v1 initial ranges)**

Sub chain recognition economics are designed to (1) price registry spam, (2) pay for real security and monitoring costs, and (3) preserve DCHUB as the security root even when a sub chain supports flexible fee denoms.
...
- Security rent (flexible gas tier, DCHUB):
  ...
  - Variable security fee: 0.1 to 1.0 DCHUB per 1,000 sub chain transactions in the anchored period, with an explicit governance cap per period
...
### 10.6B Security budget logic and scenarios (v1)
...
- **Jurisdictions and service providers**
  - Can publish smart jurisdiction modules and earn fees from entities that opt in
...
* Several smart jurisdiction modules in production.
...
* **Legal and corporate service providers**
  ...
  * Often interact with smart jurisdiction modules.
...
Relevant success indicators include:

* Number and diversity of active entities.
* Volume and quality of operations on chain.
* Adoption by NGOs and use in actual programs.
* Number and quality of smart jurisdiction and sector frameworks.
* Security metrics such as validator diversity and incident profile.
* Quality and breadth of ecosystem tools.
* Engagement with the app and module registry.
...
- **Protocol Council** (once formed)
  ...
  - Includes a reserved Founding Architect seat for an initial period, described in section 13.2A.
- **Founding Architect**
  - The individual recognized in protocol documents as the founding architect of dCorps.
...
- **dCorps foundation** (once established)
  ...
  - Coordinates development and maintenance of protocol modules, especially smart jurisdiction modules and sector frameworks.
...
- **Guardian role** (if adopted)
  - A limited, time bound role that can veto clearly harmful protocol changes within a narrow scope.
  - Any veto can be overridden by a strong supermajority of DCHUB holders.

------

### 13.2A Founding Architect seat on the Protocol Council (guardrails and thresholds)

To support continuity of vision during the formative years, one seat on the Protocol Council is reserved for the founding architect of dCorps for a limited initial period, under explicit constraints.

#### 13.2A.1 Scope and limits

- The Founding Architect seat is a single Council seat with one vote, no unilateral veto power, and no direct control over protocol state.
...
All Council members, including the Founding Architect, must publish and maintain:
...
* Approve or remove:
  ...
  * Official smart jurisdiction modules.
...
* **Grant programs**
  ...
  * Focus areas such as:
    ...
    * Smart jurisdiction modules and sector frameworks.
...
- Smart jurisdiction modules and sector frameworks are expected to begin as Wasm based modules where practical, unless they require deep consensus integration.
...
- Governance status, for example:
  - **Official module** for smart jurisdiction or sector modules that have been approved through DCHUB governance.
...
- Develops and maintains a set of **official protocol modules**, especially smart jurisdiction modules and sector frameworks.
...
- The Quarantined label must include:
  ...
  - evidence anchors (or a responsible disclosure placeholder with a defined disclosure schedule).
...
### 14.3 Smart jurisdiction modules

Smart jurisdiction modules are **protocol modules that attach to the Hub**, not separate execution layers. They implement the Module Protocol standards and can be developed by:
...
Jurisdiction adoption is expected to happen in phases. The recommended path below is designed to work immediately after mainnet launch, to speed up real world adoption, and to converge toward a final state where jurisdiction processes are directly integrated and controlled by the jurisdiction.
...
**Dispute, correction, and supersession (Step 0 standard)**

Step 0 is designed to preserve evidence quality without turning the Hub into an adjudicator.

- Entities can publish a signed dispute record that references a specific provider attestation and anchors response evidence.
- Providers can withdraw, correct, or supersede attestations with new signed statements.
- Disputed and superseded status are first class lifecycle states, not UI conventions.
...
- A jurisdiction operated or jurisdiction supervised smart jurisdiction module (Step 1 or Step 2) is live and available for that jurisdiction, or
- A formal arrangement (MoU, delegation contract, or equivalent) exists that binds registry actions to the jurisdiction module under jurisdiction controlled keys, or
...
- **Jurisdiction Module Attached (Reporting and Fees)** (Step 1)
...
This phased approach allows jurisdictions to adopt dCorps without requiring an all or nothing jump from day one, while producing practical value early. The success and legal effect of any smart jurisdiction module depends on the **law and practice** of the relevant jurisdiction, not on this Master Reference or on chain mechanics.
...
- Design and steward the Module Protocol standards that smart jurisdiction modules follow.
- Research and co design **official** smart jurisdiction modules in collaboration with local experts, regulators, and service providers.
...
- Support a broader ecosystem where third party smart jurisdiction modules can be built, listed, audited, and adopted by entities, even when they are not official.
...
* Are conducted for:
  ...
  * Official smart jurisdiction and sector frameworks.
...
#### 15.8.7 Smart jurisdiction module becomes invalid due to legal change
...
### 16.1A Release artifacts and verification checklist (v1 readiness signals)

dCorps is designed to be verifiable. The goal is that readiness can be assessed from versioned artifacts, reproducible builds, and testable behavior, not from informal assurances or private claims.

Before mainnet v1 is treated as production ready, dCorps maintains a concrete set of artifacts and verification signals.

**Code and specifications (versioned)**

- Hub chain implementation source code and build instructions.
- Protocol Specification and module standards referenced in this Master Reference:
...
- Reference tooling source code:
  - reference indexer behavior and export formats (Reference Indexer Specification),
  - reference Explorer behavior for entity pages and reporting views,
  - Compatibility Test Suite for schema and module conformance.

**Testnet and reproducibility**

- A testnet environment with:
  - published chain ID, genesis file, and validator onboarding steps,
  - tagged releases and reproducible build guidance,
  - rehearsed upgrade runs for at least one major upgrade path.
- A deterministic way for third parties (and internal reviewers) to:
  - run a node,
  - run the indexer,
  - reproduce entity views and reporting outputs from raw chain data.

**Security and audits (tracked scope and remediation)**
...
- A bug bounty program with:
  ...
  - remediation commitments and post incident reporting practices where appropriate.
...
These artifacts exist to turn readiness into observable, reviewable evidence while keeping the protocol neutral and operationally disciplined.

------

### 16.1B Team, contributors, and advisors (accountability record)

dCorps treats accountability, conflicts, and security posture as operational requirements for an internal build program, not as marketing.

**Current authorship and role**

- **Founding Architect and Author**: Nicolas Turcotte, Founder

**Internal accountability records (maintained and reviewable)**

Maintain a current, accessible record for internal governance and risk review covering:

- The legal identity and officers of the development corporation once incorporated, including jurisdiction, directors, and controlling governance documents (as applicable).
- The legal identity and governance of the dCorps foundation once incorporated, including charter, board membership, and core policies, with documented financial controls and grant processes.
- Material advisors and service providers engaged in protocol critical roles, including:
  ...
  - legal and regulatory counsel acting on protocol level structures,
    subject to confidentiality constraints in specific engagements.

**Conflict of interest and recusal standard**

- Protocol Council members and any delegated registry operators must maintain affiliation and conflict disclosures.
...
This section is about internal clarity of accountability and decision hygiene. It does not imply endorsement of any party and does not modify the protocol’s neutral, non custodial boundaries.
...
* **Infrastructure first**
  ...
  * Focus on working, useful infrastructure before worrying about listings or complex financial products.
* **Incremental decentralization**

  * Start with controlled environments while value is low.
  * Gradually widen validator sets and governance participation.
* **Regulatory awareness**

  * Design is informed by legal advice.
  * Specific offerings or modules are structured with local compliance in mind.
* **Transparency about change**

  * When plans or timelines change, reasons are explained.
* **Core stability and modular adaptation**

  * Keep the Hub’s core entity and governance logic as stable as practical.
  * Evolve protocol modules and ecosystem tools to adapt to changes in law and society.
...
- Finalize initial designs and specifications.
- Build and test prototypes on internal networks.

**Phase 1: Public testnet and mainnet v1**

- Launch public testnet for developers and early entities.
- Launch mainnet with:
  - Limited but independent validator set.
  - Hub entity modules for corporations and NGOs.
  - Basic explorers and tools.
  - Early app and module registry.
  - Reference indexer and export APIs implemented according to the Reference Indexer Specification.
  - Compatibility Test Suite for schema and module conformance, used by official modules and reference tooling.
- Onboard a first wave of Hub corporations and NGOs.

As a design intention, no public token sale or exchange listing of DCHUB is expected before:

- A public testnet has run for a meaningful period.
- Core Hub corporation and NGO modules are live on mainnet v1.
- Real entities are using basic explorers and operating with dCorps wallets.
- The reference indexer supports stable entity pages and cash based reporting views derived from the v1 reporting standard (section 9.5B).

Any actual sale or listing will be described in its own documents and will comply with applicable law. This Master Reference does not announce or offer any sale.

**Phase 2: Smart jurisdictions and first sub chains**

- Work with early jurisdiction partners on pilot smart jurisdiction modules.
- Launch the first public sub chain corporations.
- Expand tooling and ecosystem applications.
- Begin formalizing the foundation structure.

**Phase 3: Scale and fee based sustainability**

- Grow to hundreds and then thousands of entities.
- Increase protocol fee revenue to cover operational costs and validator reward budget.
- Reduce reliance on emissions via governance decisions.
- Establish the foundation and Protocol Council structures fully.
- Expand the app and module registry and formalize governance over it.
...
* **Usage**
  ...
  * Number and diversity of active entities.
  * Volume and nature of on chain operations.
...
* **Ecosystem**
  ...
  * Presence of smart jurisdiction and sector frameworks.
  * Activity in the app and module registry.
* **Financial sustainability**

  * Protocol fee revenue.
  * Treasury and foundation reserves.
  * Emissions versus fee based validator rewards.
...
- It has practical experience with crypto and Web3 projects, including token foundations and nonprofit stewards.
...
- Actively work with jurisdictions and regulators to co design and promote smart jurisdiction modules, using the Hub as a shared base layer.

A core design principle is:

- The **Hub** should remain pure, minimal Web3 infrastructure for entities.
...
- Is the neutral steward for long term public goods:
  ...
  - Official smart jurisdiction and sector modules.
...
- Delivery of funded milestones:
  ...
  - Work for jurisdictions or service providers building smart jurisdiction modules, subject to clearly disclosed terms
...
**dCorps**
 The protocol and ecosystem described in this Master Reference; a Web3 base layer for entities.
...
**Protocol module**
 An on chain module that runs on the Hub or a tightly coupled chain, reads entity and financial state, and applies additional logic. Includes smart jurisdiction modules, sector frameworks, and other rule sets. Does not change Hub consensus, but builds on top of it.

**Smart jurisdiction module**
...
* The first set of jurisdictions to adopt smart jurisdiction modules and the legal instruments they use.
...
### 18.4 Repo deliverables (v0.1 stubs required)

All referenced documents exist as versioned files in this repo.

Protocol core (normative):

- /docs/spec/SPEC-CORE.md (Protocol Specification)
- /docs/spec/SPEC-PARAMS.md (Protocol Parameters and Economics)
- /docs/spec/SPEC-MODULES.md (Module Protocol Standard)
- /docs/spec/SPEC-ANCHOR.md (Sub chain Anchoring Standard)
- /docs/spec/SPEC-INDEXER.md (Reference Indexer Specification)
- /docs/spec/SPEC-DATA.md (Data Standards, schemas, chart of accounts, tags)

Governance and policy:

- /docs/policy/POL-GOV.md (Governance Charter)
- /docs/policy/POL-TREASURY.md (Treasury Policy)
- /docs/policy/POL-REGISTRY-MODULES.md (Registry and Module Policy)
- /docs/policy/POL-VALIDATORS.md (Validator Charter)
- /docs/policy/POL-FOUNDATION.md (Foundation Charter)
...
#### Appendix A: TBD register (required)

Format: TBD-###, owner, decision needed, due trigger, linked sections/specs.

#### Appendix B: Decision log (required)

Format: DEC-###, date, decision, rationale, alternatives considered, impacted docs/specs, links.
